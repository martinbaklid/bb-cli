...
def clone() -> None:
    config = load_config()
    bitbucket = Bitbucket(
        host=config["host"], username=config["username"], token=config["token"]
    )
    for project in config["projects"]:
        ...
        for repo in repos:
            clone_urls = {link["name"]: link["href"] for link in repo["links"]["clone"]}
            repo_path = os.path.join(project_dir_path, repo["slug"])

            if os.path.exists(repo_path) and _git(
                "rev-parse", "--is-inside-work-tree", wdir=repo_path
            ):
                click.secho(
                    f"Repo {repo['slug']} is allready cloned", fg="yellow", bold=True
                )
                continue
            ...
            if not res:
                click.secho(f"Failed to clone {repo['slug']}", fg="red", err=True)
