...
def test__get_repos(mock_requests_get):
    mock_requests_get.side_effect = get_side_effect(
        {
            "http://fake.bitbucket-server.com/rest/api/1.0/projects/fake_proj/repos": FakeResponse.from_resource_path(
...
def test__get_repos_paged(mock_requests_get):
    mock_requests_get.side_effect = get_side_effect(
        {
            "http://fake.bitbucket-server.com/rest/api/1.0/projects/fake_proj/repos": FakeResponse(
                "{"
                '   "size": 2,'
                '   "limit": 2,'
                '   "isLastPage": false,'
                '   "values": [{ "slug": "fake-repo-0" }, { "slug": "fake-repo-1" }],'
                '   "start": 0,'
                '   "nextPageStart": 2'
                "}"
            ),
            "http://fake.bitbucket-server.com/rest/api/1.0/projects/fake_proj/repos?start=2": FakeResponse(
                "{"
                '   "size": 2,'
                '   "limit": 2,'
                '   "isLastPage": false,'
                '   "values": [{ "slug": "fake-repo-2" }, { "slug": "fake-repo-3" }],'
                '   "start": 2,'
                '   "nextPageStart": 4'
                "}"
            ),
            "http://fake.bitbucket-server.com/rest/api/1.0/projects/fake_proj/repos?start=4": FakeResponse(
                "{"
                '   "size": 1,'
                '   "limit": 2,'
                '   "isLastPage": true,'
